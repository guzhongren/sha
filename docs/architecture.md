...
`DiagramEnhancer.astro` reads the serialized diagram config from a JSON script tag. Mermaid blocks are rendered client-side with dynamic import from `mermaid`, with the page font stack injected through `fontFamily` and `themeVariables.fontFamily` because Mermaid writes its own `<style>` into the generated SVG. PlantUML blocks are drawn in the browser by the official `@plantuml/core` engine: an `IntersectionObserver` starts the render when the first block nears the viewport, the enhancer then loads `viz-global.js` as a classic script (the Graphviz layer the engine looks for on `globalThis.Viz`), imports the engine from its own asset URL — bundling it rewrites the state it builds at module scope — and feeds each block to `renderToString(lines, onSuccess, onError, { dark })`, serialized because the engine keeps one diagram in module state. The returned SVG is sanitized (scripts and `on*`/`javascript:` attributes removed, since a diagram may `!include` remote content) and inserted into a `figure.diagram-plantuml` marked with `data-diagram-theme`; a failure leaves the code block in place with `data-diagram-error="plantuml"`. Optional `themes` and `stdlibBase` settings load the engine's `themes.js` bundle and point `PLANTUML_STDLIB_BASE` at hosted standard-library bundles for diagrams that `!include` them. Plaintext Shiki fallback is supported by detecting blocks whose text starts with `@startuml`.
