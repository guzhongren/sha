...
The suite covers: public route inventory and draft 404s, RSS validity, home/profile and post ordering, posts pagination (numeric links and page jump input), post detail metadata/TOC/anchors, post list descriptions (two-line clamp and hover tooltip), tags and categories, the search page and global dialog (including `Cmd/Ctrl+K`), theme modes and persistence, code copy, code line numbers (gutter geometry, blank-line counting, single-line skip, `code.lineNumbers` opt-out), callouts, Mermaid/ECharts/Emoji rendering, the image viewer, link references, and mobile layout sanity checks. GitHub Actions runs it in the `e2e` job and uploads `playwright-report/` on failure.
...
When changing callout rendering or styles:

- Confirm `[!tip]`, `[!warning]`, and `[!question]` blockquotes render as `blockquote.callout.callout-*` with a matching `data-callout` attribute.
- Confirm default labels and custom titles (including the icon prefix) render as the first paragraph and raw markers never appear in the HTML.
- Confirm body text that starts on the next blockquote line (no blank line after the marker) is kept as a body paragraph instead of being folded into the title.
- Confirm multiple body paragraphs stay readable inside the panel.
- Confirm blockquotes without a marker keep their normal styling (`e2e/callouts.spec.ts`).
...
- Confirm `/posts` pagination shows numbered page links, marks the current page, and that the page-jump input navigates and clamps out-of-range values (`e2e/posts.spec.ts`).
